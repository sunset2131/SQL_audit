...
BUS-014	硬性	业务SQL内容审核	通用	预生产执行计划必须完整提供，禁止标注"无法采集"而不补采。材料存在性由MAT-009检查	检查执行计划是否为完整EXPLAIN输出	执行计划不完整：未提供完整EXPLAIN原文。	补充完整执行计划原文，或上线验证时补采后供二次确认。
BUS-015	硬性	业务SQL内容审核	通用	业务方自审结论须逐项明确回答且与证据一致。存在性由MAT-010检查。MAT-009已失败时标注"因证据缺失无法验证一致性"，不重复报错	检查自审结论5项是否与执行计划、索引证据一致	自审结论不一致：<具体矛盾描述>。	修正与执行计划、结构索引证据一致的自审结论。
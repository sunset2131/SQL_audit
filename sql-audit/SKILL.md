...
The auditor tokenizes SQL after removing comments while preserving string literals, distinguishes bound placeholders (`?`, `:name`, `$1`, `#{item}`) from `${item}` substitution, checks BUS-001 through BUS-014 in fixed order, emits each matching rule at most once, and preserves input record order. Run it even when the archive contains no SQL so the output contract remains stable. Do not create an audit JSON manually.
